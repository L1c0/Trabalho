...
// HERO · escrita (Anton), sempre com sangramento. `centro` centra na largura da tela;
// `risco` atravessa a linha com a caneta no frame dado.
export const HeroEscrito: React.FC<Base & {linhas: readonly string[]; duracao?: number; centro?: boolean; risco?: number}> = ({
  linhas,
  inicio,
  duracao,
  tamanho,
  ...
  <div style={{position: 'absolute', top: topo, ...(centro ? {left: 0, right: 0, textAlign: 'center'} : {left: esquerda})}}>
    <Sangramento estilo={estiloAnton(tamanho, cor)}>
      <LinhasEscritas linhas={linhas} inicio={inicio} duracao={duracao} />
    </Sangramento>
    {risco === undefined ? null : (
      <Risco inicio={risco} tamanho={tamanho} larguraEstimada={[...linhas[0]].length * tamanho * 0.45} cor={cor} semente={`${linhas[0]}-risco`} />
    )}
  </div>
);
